...
with app.app_context():
    ...
    user4 = User(first_name='Celestine', last_name='Mecheo', age=26, gender='Female', email='celestine@gmail.com', role='student')
    user4.set_password("Celestine123")

    user5 = User(first_name='Naomi', last_name='Achieng', age=31, gender='Female', email='naomi.achieng@gmail.com', role='instructor')
    user5.set_password("naomi20")

    user6 = User(first_name='Boniface', last_name='Muguro', age=27, gender='Male', email='boniface@gmail.com', role='student')
    user6.set_password("BonnieKim123")

    user7 = User(first_name='James', last_name='Ochieng', age=35, gender='Male', email='jamesochieng@gmail.com', role='instructor')
    user7.set_password("jimmyoch123")
    ...
    user9 = User(first_name='Kevin', last_name='Wanyonyi', age=34, gender='Male', email='kevinw@gmail.com', role='instructor')
    user9.set_password("kevinpass")

    user10 = User(first_name='Daniel', last_name='Odhiambo', age=29, gender='Male', email='dan.odhiambo@gmail.com', role='student')
    user10.set_password("dan123")

    user11 = User(first_name='Lucy', last_name='Wambui', age=24, gender='Female', email='lucywambui@gmail.com', role='student')
    user11.set_password("lucypass")
    ...
    courses_data = [
        ...
        ('Data Science with Python', 'Advanced data science concepts.', 45, 'Advanced', 15, user3.id),
        ('Web Development with Flask', 'Build web apps using Flask.', 40, 'Intermediate', 12, user5.id),
        ('Data Structures', 'Intro to data structures.', 35, 'Beginner', 10, user5.id),
        ('Machine Learning Basics', 'ML concepts explained.', 50, 'Intermediate', 14, user1.id),
        ('Deep Learning', 'Neural networks and deep learning.', 60, 'Advanced', 16, user3.id),
        ('Front-End Development', 'HTML, CSS, JS crash course.', 25, 'Beginner', 8, user7.id),
        ('React for Beginners', 'Intro to React and components.', 30, 'Beginner', 10, user5.id),
        ('APIs with Flask', 'REST API development.', 35, 'Intermediate', 11, user9.id),
        ('Database Design', 'SQL & NoSQL explained.', 30, 'Intermediate', 9, user9.id),
        ('DevOps Essentials', 'CI/CD, Docker, pipelines.', 40, 'Advanced', 12, user1.id),
        ('Cybersecurity Basics', 'Understanding digital security.', 30, 'Beginner', 7, user7.id),
    ]
    ...
    enrollments = [
        Enrollment(user_id=user2.id, course_id=1, progress=50, review_score=7, certificate_issued=True),
        Enrollment(user_id=user4.id, course_id=3, progress=80, review_score=8, certificate_issued=True),
        Enrollment(user_id=user6.id, course_id=2, progress=60, review_score=9, certificate_issued=True),
        ...
        Enrollment(user_id=user10.id, course_id=4, progress=75, review_score=9, certificate_issued=True),
        Enrollment(user_id=user11.id, course_id=5, progress=90, review_score=8, certificate_issued=True),
        Enrollment(user_id=user12.id, course_id=7, progress=65, review_score=9, certificate_issued=True),
        Enrollment(user_id=user13.id, course_id=1, progress=55, review_score=7, certificate_issued=True),
        Enrollment(user_id=user14.id, course_id=3, progress=45, review_score=7, certificate_issued=False),
        Enrollment(user_id=user15.id, course_id=2, progress=80, review_score=9, certificate_issued=True),
        Enrollment(user_id=user11.id, course_id=9, progress=60, review_score=8, certificate_issued=True),
        Enrollment(user_id=user10.id, course_id=10, progress=40, review_score=7, certificate_issued=False),
        Enrollment(user_id=user12.id, course_id=12, progress=85, review_score=9, certificate_issued=True),
        Enrollment(user_id=user13.id, course_id=11, progress=70, review_score=8, certificate_issued=True),
    ]
    ...
    reviews = [
        Review(user_id=user2.id, course_id=1, rating=4.5, comment='Really helpful.'),
        Review(user_id=user4.id, course_id=3, rating=4.0, comment='Well paced.'),
        Review(user_id=user6.id, course_id=2, rating=5.0, comment='Excellent content!'),
        Review(user_id=user8.id, course_id=6, rating=4.2, comment='Deep learning made simple.'),
        Review(user_id=user10.id, course_id=4, rating=4.8, comment='Great intro to data structures.'),
        Review(user_id=user12.id, course_id=7, rating=4.0, comment='Fun and interactive.'),
        Review(user_id=user11.id, course_id=5, rating=4.7, comment='ML was surprisingly clear.'),
        Review(user_id=user14.id, course_id=3, rating=4.9, comment='Loved the Flask project.'),
        Review(user_id=user13.id, course_id=1, rating=4.4, comment='Well explained basics.'),
        Review(user_id=user15.id, course_id=2, rating=5.0, comment='Highly recommended.'),
        Review(user_id=user10.id, course_id=10, rating=4.1, comment='DB examples were useful.'),
        Review(user_id=user13.id, course_id=11, rating=4.3, comment='DevOps now makes sense.')
    ]
